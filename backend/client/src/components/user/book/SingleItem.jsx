...
class SingleItem extends Component {
  ...
  getMediaInfo = () => {
    ...
    axios
    .get(`/api/book/details/${bookmark.media_id}`)
    .then(res => {
      this.setState({
        media: res.data,
  ...
  handleButton = e => {
    ...
    axios
    .post('/users/book/progress', { progress: progress, id: id })
    .then(res => {
      this.setState({
        progress: progress
      })
  ...
  render() {
    ...
    return (
      <div className='SingleItem' >
        <img className='list-img' src={media.volumeInfo.imageLinks.small} alt='poster'/>
        <div>
          <div>
          ...
          <div className='progressbar-container' style={{width:'300px', height:'25px', backgroundColor:'black'}} >
            <div className='progressbar' style={{width: `${(progress/media.volumeInfo.printedPageCount) * 300}px`, height: '25px', backgroundColor:'darkgreen'}}/>
          </div>
          <div>
            {
              edit
            ...
            /{media.volumeInfo.printedPageCount}
          </div>
          {
            edit
              ? <div>
